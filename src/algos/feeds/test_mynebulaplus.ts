...
export const handler = async (ctx: AppContext, params: QueryParams, userDid: string, follows?: string[]) => {
    ...
    if (!existingRank1 || !existingRank2) {
        res = await getFirstPagePosts(ctx, { withExplore: false, seed, gravity: 3, limit: params.limit * 3 }, communityResponseWithoutExplore);
        lastRank1 = 99999999;
        lastRank2 = 99999999;
    } else {
        ...
        res = await mergePosts(seed, 10, rateLimit(res), rateLimit(res2));
    }
