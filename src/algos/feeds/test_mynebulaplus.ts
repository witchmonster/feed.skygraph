...
export const handler = async (ctx: AppContext, params: QueryParams, userDid: string, follows?: string[]) => {
    ...
    if (!existingRank1 || !existingRank2) {
        res = await getFirstPagePosts(ctx, params.limit * 2, 3, communityResponseWithoutExplore);
        lastRank1 = 99999999;
        lastRank2 = 99999999;
    } else {
        res = await getRankedPosts(ctx, existingRank1, params.limit * 2, 3, communityResponse);
        lastRank1 = res?.at(-1).rank;
        const res2: any = await getRankedPosts(ctx, existingRank2, params.limit * 2, 4, communityResponseWithoutExplore);
